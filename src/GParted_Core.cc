			     ...
			     partitions[ t ] .type == GParted::TYPE_LOGICAL ) 
			{
				if ( partitions[ t ] .busy )
				{
					switch( get_fs( partitions[ t ] .filesystem ) .online_read )
					{
						case FS::EXTERNAL:
						{
							FileSystem* p_filesystem = set_proper_filesystem( partitions[ t ] .filesystem ) ;
							if ( p_filesystem )
...
bool GParted_Core::commit( PedDisk* lp_disk )
{
	...
	succes = commit_to_os( 10, lp_disk ) && succes ;

	return succes ;
}

bool GParted_Core::commit_to_os( std::time_t timeout, PedDisk* lp_disk )
{
	bool succes ;
#ifndef USE_LIBPARTED_DMRAID
	DMRaid dmraid ;
